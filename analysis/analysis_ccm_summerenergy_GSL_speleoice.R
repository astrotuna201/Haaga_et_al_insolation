# This script performs CCM between summer energy and GSL on the age model 
# with orbitally independent age assignments ('speleoice'). 
...
# 
# NB! Each result file is around 41 Mb when using the analysis setup outline 
# in the paper. For a total of 3433 files, that amounts to almost 200 Gb. 
...
#########################
# Symmetric lags of 12 kyr are necessary for results after the lagtest to 
# converge. The corresponding maximum lag for the LR04 age model is 8 kyr,
# possibly reflecting the build-in lag after insolation in the LR04 age model.
lags <- -12:12  
...
lib <- c(1, dim(data)[1])
pred <- lib 
samples.original <- 800
samples.surrogates <- 200
n.surrogates <- 400
    ...
    for (t in thresholds) {
      ...
      if (frac.zeros > 0.02) {
        ...
        df <- subset(x = summer.energy.data,
                    ...
                    select = c("SummerEnergy", "GSLSpeleoIce")) %>% 
          as.data.frame
        ...
        df$threshold <- rep(t)
        filename <- paste("SummerEnergyDrivesGSLSpeleoIce_", 
                          toString(t), "_", toString(l), ".RData", sep = "")
        
        path <- paste("results/crossmap_summerenergy_GSL_speleoice/", 
                      filename, sep = "")
